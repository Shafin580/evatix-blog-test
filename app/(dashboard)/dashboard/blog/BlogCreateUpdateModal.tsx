...
import { fileToBase64, validateTextInputField } from "@/lib/utils";
...
const BlogCreateUpdateModal = ({
  ...
  const checkDataValidation = (data: BlogItemProps) => {
    ...
    if (data.tags?.length == 0) {
      setErrors((prev) => ({
        ...prev,
        tagsError: "Please insert atleast one tag",
      }));
      status = false;
    }
        ...
        <div className="flex flex-col space-y-6">
          ...
          <RichTextEditor
            ...
            onChange={(value) => {
              setFormData((prev) => ({
                ...prev,
                describeProblem: value == "<p></p>" ? "" : value,
              }));
          ...
          <MultipleFileUploadCustom
            ...
            getFiles={async (files: any) => {
              if (files) {
                ...
                console.log("uploaded base 64 image", base64File, files[0]);
                setFormData((prev) => ({ ...prev, featureImage: base64File }));
              }
            }}
            maxUploadSize={10}
