import { ClassNamesConfig, GroupBase } from "node_modules/react-select/dist/declarations/src"
...
const SelectCreatableMulti = (props: SelectCreatableMultiProps) => {
  ...
  // + custom styles
  const customStyles:
    | ClassNamesConfig<SelectCreatableMultiValueType, true, GroupBase<SelectCreatableMultiValueType>>
    | undefined = {
    ...
    multiValue: () => cn("whitespace-nowrap !bg-slate-200 !font-semibold hover:!bg-slate-300 !rounded-full"),
    multiValueLabel: () => cn("!pl-8 -mr-2 !text-slate-500"),
  }

  // + custom components
  const customComponents:
    | Partial<SelectComponents<SelectCreatableMultiValueType, true, GroupBase<SelectCreatableMultiValueType>>>
    | undefined = {
        ...
        {isClearable && hasValue && (
          ...
          >
            <Icon
              iconName="x"
              iconSize="19"
              className={cn("stroke-slate-400 transition-colors duration-200 hover:stroke-rose-600", {
  ...
  return (
    <div className={cn("altd-select-creatable-multi relative w-full transition", containerClassName)}>
